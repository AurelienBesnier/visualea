    ...
    if not pyfilename.exists():
        ...
        if mtime_py > mtime_ui:
            ...
            with open(pyfilename, 'r') as f:
                content = f.read()
                generate = QT_MODULE_NAME.lower() not in content.lower()
        else:
            generate = True
...
## Old implementation to test if ui files exist or if Qt changed, I think it is not necessary because we do
## the test line 175 `generate = QT_MODULE_NAME.lower() not in content.lower()`
# try:
